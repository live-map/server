...
### 일반적인 OAuth 2.0 Authorization Code Flow
...
일반적인 OAuth 다이어그램에서 "서비스 Client"는 하나의 서버입니다. Grapoll은 프론트엔드(Next.js)와 백엔드(FastAPI)가 분리되어 있어, "서비스 Client"가 두 서버로 나뉩니다.

프론트엔드 콜백 라우트가 존재하는 이유: 백엔드(`localhost:8000`)가 프론트엔드 도메인(`localhost:3000`)에 쿠키를 설정할 수 없기 때문입니다. 프론트엔드 콜백은 쿠키 설정만 담당하는 프록시 역할을 합니다.

```
Browser              Next.js (:3000)              FastAPI (:8000)              Google
  │                       │                            │                         │
  │  1. 로그인 버튼 클릭    │                            │                         │
  │──────────────────────>│                            │                         │
  │                       │  2. OAuth 인증 URL 요청      │                         │
  ...
  │<──────────────────────│                            │                         │
  │                       │                            │                         │
  │  5. Google 로그인 페이지에서 계정 선택 및 권한 동의                               │
  │─────────────────────────────────────────────────────────────────────────────>│
  │                       │                            │                         │
  │  6. Authorization Code와 함께 redirect_uri로 리다이렉트                         │
  ...
  │                       │                            │  sessions 테이블에 저장   │
  │                       │                            │                         │
  │                       │  16. JWT + Refresh Token    │                         │
  │                       │  반환                       │                         │
  │                       │<───────────────────────────│                         │
  │                       │                            │                         │
  │                       │  17. JWT를 httpOnly 쿠키로   │                         │
  │                       │  설정 (프론트엔드 도메인)     │                         │
  │                       │                            │                         │
  │  18. 쿠키 설정 +       │                            │                         │
  │  홈페이지로 리다이렉트   │                            │                         │
...
> **Note**: Provider의 Access Token은 백엔드에서만 처리됩니다 (9-12단계). 프론트엔드는 Authorization Code(단일 사용, client_secret 없이 무용)만 중계하고, 백엔드가 발급한 자체 JWT만 쿠키로 저장합니다.

아래 각 Phase 설명에서 **백엔드가 담당하는 부분**(2, 8-15단계)을 중심으로 코드를 매핑합니다.
