...
LiveMap은 다중 소스(12개 구현됨, 현재 2개 활성화)에서 글로벌 이벤트를 모니터링하고, 다중 소스 교차 검증을 통해 정보를 검증하며, 실시간으로 이중 언어(한국어/영어) 뉴스 기사를 생성하는 자율 뉴스 인텔리전스 시스템입니다.
...
│    현재: GDELT, Reddit (12개 소스 구현됨)                        │
├─────────────────────────────────────────────────────────────────┤
│ 2. 4-LAYER DATE FILTERING (신규)                                │
│    URL 날짜 → 최신성 필터 → 콘텐츠 날짜 → 과거 연도 체크        │
├─────────────────────────────────────────────────────────────────┤
...
과거 기사가 새 기사로 발행되는 것을 방지:

| 레이어 | 기능 | 예시 |
|--------|------|------|
| **URL 날짜 추출** | 기사 URL에서 날짜 추출 | `/2023/10/15/article` → 2023-10-15 |
| **최신성 필터** | 임계값보다 오래된 이벤트 거부 | > 48시간 → 거부 |
| **콘텐츠 날짜 감지** | 기사 본문에서 날짜 탐지 | "2023년 10월 15일" 감지 |
| **과거 연도 체크** | 과거 연도 언급 감지 | "2023년에..." → 거부 |

**주요 함수:**
- `extract_date_from_content()` - 기사 텍스트에서 날짜 추출
- `contains_past_year()` - 과거 연도 언급 감지
- `validate_trigger_recency()` - URL 기반 날짜 검증

### Source Tier 시스템

| Tier | 소스 | 신뢰도 | 신뢰 수준 | 상태 |
|------|---------|-------------|-------------|--------|
| **Tier-1 정부** | USGS, NOAA | 0.99 | 즉시 게시 | 비활성화 (범위 외) |
| **Tier-1 뉴스** | GDELT (Reuters, AP, BBC) | 0.90 | 게시 가능 | **활성화** |
| **Tier-2 데이터** | ACLED | 0.85 | 추가 확인 필요 | 비활성화 (API 키 필요) |
| **Tier-2 뉴스** | Currents, WorldNews | 0.75 | 추가 확인 필요 | 비활성화 (API 키 필요) |
| **Tier-3 소셜** | Reddit | 0.40 | 신호만 | **활성화** |
| **Tier-3 소셜** | Bluesky, Google Trends | 0.40 | 신호만 | 비활성화 (인증/속도제한 문제) |
| **Tier-3 메시징** | Telegram | 0.35 | 신호만 | 비활성화 (설정 필요) |
...
| 감지 지연 | < 15분 | 15분 (폴링 기반) |
| 오탐률 | < 5% | 미측정 |
| 소스 커버리지 | 12개 소스 | 2개 활성화 (GDELT, Reddit) |
| 검증 정확도 | > 90% | 미측정 |
| LLM 비용 절감 | > 80% | 이론적 (측정 필요) |
